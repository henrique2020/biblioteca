...
$livro = Livro::buscarPorID($id, 'completo');
$slug = slugificar($livro->livro);
$lista_navegacao = [
    ['nome' => 'Home', 'link' => '/'],
    ['nome' => "Livro: {$livro->livro}", 'link' => "/livro/{$slug}"],
    ['nome' => 'Editar', 'link' => '']
];
?>
...
<body>
            ...
            <div class="card-body">
                <form id="cadastro">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="livro" name="livro" value="<?= $livro->livro ?>" required>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control" id="data-lancamento" name="data-lancamento" value="<?= $livro->dataLancamento->format('Y-m-d') ?>" max="<?= date('Y-m-d') ?>" required>
                    </div>
                    <div class="mb-3">
    ...
    <script>
        // Gêneros atuais do livro
        const generosAtuais = <?= $livro->generos ? json_encode(array_map(fn($g) => ['id' => $g->id, 'value' => $g->genero], $livro->generos)) : '[]' ?>;
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                // Preenche o Tagify com os gêneros atuais do livro
                if (generosAtuais && generosAtuais.length > 0) {
                    let tagsInicial = generosAtuais.map(genero => {
                        let encontrado = generos.find(g => g.id == genero.id || g.value === genero.value);
                        return encontrado || { value: genero.value, id: genero.id };
                    });
                    tagify.addTags(tagsInicial);
                }
            }
        });
        ...
        $('form').on('submit', function(event) {
            ...
            $.ajax({
                ...
                success: function(retorno) { 
                    if(retorno.ok){
                        emiteAviso('Livro editado com sucesso!', 'success');
                ...
                error: function() { 
                    emiteAviso('Não foi possível editar este livro. Tente novamente mais tarde', 'danger'); 
                },
                complete: function() { 
